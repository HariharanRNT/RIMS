    ...
    <div>
      ...
      {report && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.85rem', marginBottom: '1.5rem' }}>
          ...
          <div className="ui-card" style={{ padding: '0.85rem 1rem', background: report.totalLOPDays > 0 ? 'var(--danger-bg)' : undefined, border: report.totalLOPDays > 0 ? '1px solid rgba(239, 68, 68, 0.3)' : undefined }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Total LOP Days</div>
            <div style={{ fontSize: '1.35rem', fontWeight: 800, color: report.totalLOPDays > 0 ? 'var(--danger-text)' : 'var(--success-text)', marginTop: '0.15rem' }}>{report.totalLOPDays}</div>
          </div>

          <div className="ui-card" style={{ padding: '0.85rem 1rem', background: report.totalLOPAmount > 0 ? 'var(--danger-bg)' : undefined, border: report.totalLOPAmount > 0 ? '1px solid rgba(239, 68, 68, 0.3)' : undefined }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>LOP Amount</div>
            <div style={{ fontSize: '1.35rem', fontWeight: 800, color: report.totalLOPAmount > 0 ? 'var(--danger-text)' : 'var(--success-text)', marginTop: '0.15rem' }}>
          ...
          <table className="data-table">
            ...
            <tbody>
              {report.dailySummaries.map((item) => {
                ...
                return (
                  <tr key={item.date} style={{ backgroundColor: item.isSandwichLeave ? 'var(--warning-bg)' : undefined }}>
